...
import { Clock, Calendar, Users, AlertCircle, CheckCircle, XCircle, Coffee, Eye } from 'lucide-react'
...
export function AttendanceCard({ attendanceData, periodLabel, employee }: AttendanceCardProps) {
  ...
  const calculateSummary = () => {
    if (!attendanceData.logs) return { totalWorked: 0, daysPresent: 0, daysAbsent: 0, totalLate: 0 }
    
    const totalWorked = attendanceData.logs.reduce((sum, log) => sum + log.worked, 0)
    const daysPresent = attendanceData.logs.filter(log => !log.isEmpty && log.worked > 0).length
    const daysAbsent = attendanceData.logs.filter(log => log.isEmpty || log.hoursAbsent > 0).length
    const totalLate = attendanceData.logs.reduce((sum, log) => sum + log.late, 0)
    
    return { totalWorked, daysPresent, daysAbsent, totalLate }
  }
  ...
  const getStatusIcon = (log: AttendanceLog) => {
    if (log.isLeave || log.isHoliday || log.isRestDay) return <Coffee className="h-4 w-4 text-blue-500" />
    if (log.isEmpty || log.isError) return <XCircle className="h-4 w-4 text-red-500" />
    if (log.worked > 0) return <CheckCircle className="h-4 w-4 text-green-500" />
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="bg-green-50 p-3 rounded-lg border border-green-200">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs font-medium text-green-800">Days Present</span>
            </div>
          ...
          <div className="bg-red-50 p-3 rounded-lg border border-red-200">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs font-medium text-red-800">Days Absent</span>
            </div>
          ...
          <div className="bg-orange-50 p-3 rounded-lg border border-orange-200">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs font-medium text-orange-800">Late Hours</span>
            </div>
                    ...
                    <div>
                      ...
                      <div className="text-xs text-gray-600">
                        {log.inTime || log.outTime ? (
                          <>
                            In: {formatTime(log.inTime)} • Out: {formatTime(log.outTime)}
                          </>
                        ) : (
